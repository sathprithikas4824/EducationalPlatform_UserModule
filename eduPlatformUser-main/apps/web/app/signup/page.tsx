...
export default function SignupPage() {
  ...
  const failSignup = (msg: string) => {
    setError(msg);
    announce(`Sign up failed. ${msg}`, "assertive");
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      failSignup("Password must be at least 6 characters.");
      return;
    }
    ...
    try {
      ...
      if (signUpError) {
        ...
        if (!isSmtpFailure) {
          ...
          failSignup(signUpError.message);
          return;
        }
      ...
      failSignup("An unexpected error occurred. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "linear-gradient(135deg, #f5f3ff 0%, #faf5ff 50%, #f0f9ff 100%)" }}>
        <div className="w-full max-w-md">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-8" style={{ boxShadow: "0 4px 24px 0 rgba(124, 58, 237, 0.08), 0 1px 4px 0 rgba(0,0,0,0.06)", border: "1px solid rgba(140, 140, 170, 0.2)" }}>
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-1 text-center">Confirm your email</h2>
            <p className="text-gray-500 text-sm text-center mb-1">We sent a confirmation link to</p>
            <p className="font-semibold text-gray-800 text-center mb-6">{email}</p>

            <p className="text-sm text-gray-500 text-center mb-5">
              Click the link in the email to activate your account, then{" "}
              <Link href="/login" className="font-semibold hover:underline" style={{ color: "#7a12fa" }}>sign in</Link>.
            </p>

            <div className="text-center mb-4">
              {resendStatus === "sent" ? (
                <p className="text-sm font-medium text-green-600">Email resent! Check your inbox and spam folder.</p>
              ) : (
                <button
                  type="button"
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-8" style={{ boxShadow: "0 4px 24px 0 rgba(124, 58, 237, 0.08), 0 1px 4px 0 rgba(0,0,0,0.06)", border: "1px solid rgba(140, 140, 170, 0.2)" }}>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              <label htmlFor="signup-full-name" className="block text-sm font-medium text-gray-700 mb-1.5">Full name</label>
              <input
                id="signup-full-name"
                type="text"
                value={fullName}
